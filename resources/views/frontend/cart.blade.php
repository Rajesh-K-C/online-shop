                                ...
                                @forelse($data['carts'] as $cart)
                                    <tr>
                                        ...
                                        <td class="shoping__cart__price">
                                            {{ formatFloat(($cart->product->price / 100) * (100 - $cart->product->discount_percent)) }}
                                        </td>
                                        <td class="shoping__cart__quantity">
                                            <div class="quantity">
                                                <div class="pro-qty">
                                                    <input type="text" min="0" data-cart-id="{{ $cart->id }}"
                                                        max="{{ $cart->product->stock }}" data-product-id="{{ $cart->id }}"
                                        ...
                                        <td class="shoping__cart__total">
                                            {{ formatFloat((($cart->product->price / 100) * (100 - $cart->product->discount_percent)) * $cart->quantity) }}
                                        </td>
...
<script>
    ...
    document.querySelectorAll('.pro-qty input').forEach(input => {
        ...
        const price = Number.parseFloat(all.children[1].innerText);


        qty.firstElementChild.addEventListener('click', () => {
            if (parseInt(input.value) > 0) {
                input.value = parseInt(input.value) - 1;
                all.children[3].innerText = price * parseInt(input.value);
                updateCart(input);
            }
        });
    ...
    function calculateTotal() {
        document.querySelectorAll('.shoping__cart__total').forEach(e => {
            total += Number.parseFloat(e.innerText);
        });
